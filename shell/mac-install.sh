...
# brewのインストール（Apple Silicon Mac用）
install_brew() {
...
# nvmをbrewでインストール（コマンドが存在しない場合でもformulaの有無で判定）
install_nvm_with_brew() {
    ...
    log_info "Installing nvm with Homebrew..."
    if brew install nvm; then
        # brew nvm formulaのcaveatに従って NVM_DIR ディレクトリを作成
        mkdir -p "$HOME/.nvm"
...
# oh-my-zshのインストール（brew formulaが存在しないためcurl）
install_ohmyzsh() {
...
# oh-my-zshプラグインのインストール（$ZSH_CUSTOM/plugins/ に配置するためgit clone）
...
main() {
    echo "=========================================="
    echo "  Dotfiles Installation Script (macOS)"
    echo "=========================================="
    echo ""
    ...
    # oh-my-zshのインストール（brew formulaが無いためcurl）
    install_ohmyzsh
